...
final class SequencerManager
{
     ...
     */
    public function execute(string $operation, bool $async = false, bool $record = true): void
    {
        $instance = $this->loadOperation($operation);
        $operationName = $this->resolveOperationName($operation);

        if ($record) {
            $this->executeWithRecord($instance, $operationName, $async);
        } else {
     ...
     */
    private function executeWithRecord(Operation $operation, string $operationName, bool $async): void
    {
        ...
        $record = OperationModel::query()->create([
            'name' => $operationName,
            'type' => $async ? 'async' : ExecutionMethod::Sync,
            'executed_at' => Date::now(),
        ]);
